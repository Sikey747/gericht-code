...
import { isMobile, getHash, getDigFormat }  from './../../js/files/functions.js';
import  { menuClose } from './../menu-icon/menu-icon.js';
	...
	document.addEventListener("windowScroll", function (e) {
		...
		if (scrollTop >= startPoint) {
			...
			if (headerShow) {
				...
				// разкоментировать если надо что бы хедер возвращася сразу после скролла
				// timer = setTimeout(() => {
				// 	!header.classList.contains('_header-show') ? header.classList.add('_header-show') : null;
				// }, headerShowTimer);
			}
